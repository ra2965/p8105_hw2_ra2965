...
There are `r nrow(precipitation)` observations and `r ncol(precipitation)` in the precipitation dataset. The dataset includes key variables of `r colnames(precipitation)`, which are the precipitation data for each month in 2017 and 2018.
...
The cleaned file “pols-month” contains `r nrow(pols)` observations of `r ncol(pols)` variables related to the number of national politicians who are democratic or republican at any given time. The key variables in this dataset are `r colnames(pols)`.

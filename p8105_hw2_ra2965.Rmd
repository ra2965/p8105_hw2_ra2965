...
```
There are `r nrow(trash_wheel)` observations in the Mr. Trash Wheel dataset and there are `r ncol(trash_wheel)` variables.

There are `r nrow(precipitation)` observations in the precipitation dataset. For available data, the total precipitation in 2018 is `r sum(pull(precipitation_2018, total))`. The median number of sports balls in a dumpster in 2017 is `r median(pull(trash_wheel, sports_balls))`.
...
baby_names %>% filter(gender == "MALE", childs_first_name == "ethan") %>% select(childs_first_name, year_of_birth,ethnicity, rank) %>%
  pivot_wider(names_from = "year_of_birth",
              values_from = "rank") %>% knitr::kable()
```
